...
async function processDownloads() {
    ...
    const sizesOutputPath = join(__dirname, '../src/data/download-sizes.json');
    writeFileSync(sizesOutputPath, JSON.stringify(sizes, null, 2));
    console.log(`\nSaved ${Object.keys(sizes).length} sizes to download-sizes.json`);

    const urlsOutputPath = join(__dirname, '../public/downloads.txt');
    writeFileSync(urlsOutputPath, Object.keys(sizes).join('\n'));
    console.log(`\nSaved ${Object.keys(sizes).length} urls to download.txt`);
}
