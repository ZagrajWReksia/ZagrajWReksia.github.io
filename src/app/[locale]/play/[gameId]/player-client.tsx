...
export default function PlayerClient({ gameId }: { gameId: string }) {
  ...
  }, [ready, playthroughId, game, migrateSaveFile]);
    ...
    <Container>
      ...
      <Controls>
        <Button onClick={enterFullscreen}>
          <Fullscreen size={15} /> {t('playerEnterFullscreen')}
        </Button>
        <Button onClick={exportSaveFile} disabled={!ready}>
          <FileDown size={15} /> {t('playerExportSaveFile')}
        </Button>
        <Button onClick={importSaveFile} disabled={!ready}>
      ...
      <FooterStyled>
        Powered by{' '}
        <a href="https://github.com/ReksioEngine/ReksioEngine">ReksioEngine</a>
      </FooterStyled>
    </Container>
  );
}
